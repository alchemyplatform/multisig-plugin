...
import {FactoryHelpers} from "@alchemy/modular-account/src/helpers/FactoryHelpers.sol";
import {IEntryPoint} from "@alchemy/modular-account/src/interfaces/erc4337/IEntryPoint.sol";
import {IAccountInitializable} from "@alchemy/modular-account/src/interfaces/IAccountInitializable.sol";

/// @title Multisig Plugin Modular Account Factory
/// @author Alchemy
/// @notice Factory for upgradeable modular accounts with MultisigPlugin installed.
...
contract MultisigModularAccountFactory is Ownable2Step {
    IEntryPoint public immutable ENTRYPOINT;
    address public immutable MULTISIG_PLUGIN;
    address public immutable IMPL;
    bytes32 internal immutable _MULTISIG_PLUGIN_MANIFEST_HASH;
    ...
    constructor(
        ...
        bytes32 multisigPluginManifestHash,
        IEntryPoint entryPoint
    ) {
        _transferOwnership(owner);
        MULTISIG_PLUGIN = multisigPlugin;
        IMPL = implementation;
        _MULTISIG_PLUGIN_MANIFEST_HASH = multisigPluginManifestHash;
        ...
        returns (address addr)
    {
        ...
        if (addr.code.length == 0) {
            ...
            plugins[0] = MULTISIG_PLUGIN;

            bytes32[] memory manifestHashes = new bytes32[](1);
            manifestHashes[0] = _MULTISIG_PLUGIN_MANIFEST_HASH;
